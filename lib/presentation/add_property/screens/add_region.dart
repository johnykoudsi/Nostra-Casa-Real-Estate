...
class _AddRegionState extends State<AddRegion> {
  ...
  @override
  void initState() {
    CountryModel? selectedCountryBloc =
        context.read<AddPropertyBloc>().state.countryModel;
    ...
    return BlocListener<CountryBloc, CountryState>(
      listener: (context, state) {
        if(state is CountryErrorState){
          DialogsWidgetsSnackBar.showSnackBarFromStatus(context: context, helperResponse: state.helperResponse);
        }
      },
      child: Scaffold(
            ...
            children: [
              ...
              Expanded(
                child: RefreshIndicator(
                  onRefresh: () async {
                    context.read<CountryBloc>().add(GetCountryCityEvent());
                  },
                  child: ListView(
                    children: [
                      Text(
                                ...
                                onChanged: (value) {
                                  setState(() {
                                    ...
                                    context
                                        ...
                                        .add(SelectCountryRegionEvent(
                                          countryModel:
                                              state.getCountryFromName(
                                                  selectedCountry),
                                          city: null,
                                        ));
                                  });
                        ...
                        builder: (context, state) {
                          if (state is CountryDoneState &&
                            ...
                            List<String> cityList() {
                              List<String> stringCity = [];
                              for (var element in state.getCityByCountryName(
                                  selectedCountry ?? '')) {
                                stringCity.add(element.name);
                              }
                              return stringCity;
